...
The optional `runninghub` provider has a configuration-driven submit/poll adapter. Real network calls are disabled by default and require explicit `RUNNINGHUB_ENABLE_REAL_CALL=true` plus backend-only Key, submit/poll endpoints, workflow/app identifiers, and node input mapping. No RunningHub Key belongs in frontend configuration, Electron resources, Git, README examples, or logs. Missing or invalid configuration fails safely and directs users to the local AI mask or manual mask.
